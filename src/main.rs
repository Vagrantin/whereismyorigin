...
fn main() -> io::Result<()> {
            ...
            "--timeout" => {
                if i + 1 < args.len() {
                    match args[i + 1].parse::<u64>() {
                        ...
                        Err(_) => {
                            eprintln!("Error : Invalid timeout value '{}'. Please provide a number in minutes.", args[i + 1 ]);
                            process::exit(1);
                        }
                    }
                } else {
        ...
        match result {
            Ok((key_bytes, _value_bytes)) => {
                if let Ok(videofilepathb64) = String::from_utf8(key_bytes.to_vec()) {
                    ...
                    let has_match =
                        process_file_with_mxfdump(&videofilepath, verbose, mxferror, timeout)?;
                    ...
                    if has_match {
                        ...
                        let _ = db.insert(
                            &videofilepathb64.as_bytes(),
                            originpresent.to_string().as_bytes(),
                        );
    ...
    println!(
        "Processing complete. Processed {} files total.",
        processed_count
    );
    println!(
        "Found Origin/Precharge pattern in {} files.",
        found_matches_count
    );
    Ok(())
}

fn process_file_with_mxfdump(
    file_path: &str,
    verbose: bool,
    process_errors: bool,
    timeout_minutes: u64,
    ...
    let mxf_opatom_pattern =
        r"Operational\s+Pattern\s+=\s+06.0e.2b.34.04.01.01.02.0d.01.02.01.10.02.00.00";
...
fn is_video_file(
    mxf_dump_chunk: &str,
    file_path_clone: &str,
    audio_container_regex: &Regex,
    audio_opatom_regex: &Regex,
) -> bool {
